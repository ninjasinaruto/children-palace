    ...
    public partial class SalaryAddForm : Form
    {
        ...
        private void SalaryAddForm_Load(object sender, EventArgs e)
        {
            ...
            this.teacherSalaryLogTableAdapter.FillByLastMonthWithTeacherID(this.teacherWageDataSet.TeacherSalaryLog, TeacherID);
            {
                ...
                if (teacherWageDataSet.TeacherSalaryLog.Rows.Count > 0)
                {
                    int logID = teacherWageDataSet.TeacherSalaryLog.Rows[0].Field<int>("LogID");
                    // 基本工资
        ...
        {
            this.teacherSalaryLogTableAdapter.FillByThisMonthWithTeacherID(this.teacherWageDataSet.TeacherSalaryLog, dtpBasic.Value, TeacherID);
            if (this.teacherWageDataSet.TeacherSalaryLog.Count > 0)
            {
                MessageBox.Show("该教师当月已经添加过工资信息，请勿重复添加！");
                return;
            }
            else
            {
